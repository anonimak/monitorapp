...
class UservpnController extends Controller
{
    ...
    public function detail(Request $request, $ip)
    {
        ...
        return Inertia::render('Uservpn/detail', [
            'dataClient' => $client,
            'dataClientDnsAllowed' => ClientDNS::getData($ip, [2, 3], $request->input('search'))->paginate(10),
            'dataClientDnsBlocked' => ClientDNS::getData($ip, [1, 4], $request->input('search'))->paginate(10),
            'perPage' => 10,
            'filters' => $request->all(),
            'breadcrumbItems' => array(
                [
